...
library LibSet_uint256
{
	...
	function indexOf(set storage _set, uint256  _value)
	internal view returns (uint256)
	{
		return _set.indexes[_value];
	}

	function contains(set storage _set, uint256  _value)
	...
	function add(set storage _set, uint256  _value)
	...
	function remove(set storage _set, uint256  _value)
	internal returns (bool)
	{
		if (!contains(_set, _value))
		{
